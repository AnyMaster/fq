...
var serialTypeMap = scalar.SToSymStr{
	serialTypeNULL:       "null",
	serialTypeS8:         "s8",
	serialTypeSBE16:      "sbe16",
	serialTypeSBE24:      "sbe24",
	serialTypeSBE32:      "sbe32",
	serialTypeSBE48:      "sbe48",
	serialTypeSBE64:      "sbe64",
	serialTypeFloatBE64:  "floatbe64",
	serialTypeInteger0:   "integer0",
	serialTypeInteger1:   "integer1",
...
func sqlite3CellPayloadDecode(d *decode.D, h sqlite3Header) {
	...
	d.LenFn((length)*8-lengthBits, func(d *decode.D) {
		d.FieldArray("serials", func(d *decode.D) {
			for !d.End() {
				serialTypes = append(serialTypes, d.FieldSFn("serial", varintDecode, scalar.Fn(func(s scalar.S) (scalar.S, error) {
					typ := s.ActualS()
...
func sqlite3DecodeTreePage(d *decode.D, h sqlite3Header, x int64, payLoadLen int64) {
	...
	if p <= x {
		...
		d.LenFn(firstPayLoadLen*8, func(d *decode.D) {
		...
		d.FieldArray("overflow_pages", func(d *decode.D) {
			...
			d.FieldStruct("overflow_page", func(d *decode.D) {
				bib := d.FieldRawLen("data", firstPayLoadLen*8)
				nextPage = d.FieldS32("next_page")

				bb, err := bib.Bytes()
				if err != nil {
					d.IOPanic(err, "first overflow page")
				}
				payLoadBB.Write(bb)
			})
			...
			for nextPage != 0 {
				...
				d.FieldStruct("overflow_page", func(d *decode.D) {
					...
					bib := d.FieldRawLen("data", overflowSize*8)
					payLoadLenLeft -= overflowSize

					bb, err := bib.Bytes()
					if err != nil {
						d.IOPanic(err, "overflow page")
					}
					payLoadBB.Write(bb)
				})
			}
		...
		d.FieldStructRootBitBufFn("payload",
			bitio.NewBufferFromBytes(payLoadBB.Bytes(), -1),
